...
app.controller("shoppping-cart-ctrl", function($scope, $http) {
    ...
    **/
    $scope.cart = {
        ...
        clear() {
        	var result = confirm("Bạn có muốn xóa tất cả mặt hàng ?");
            if (result) {
	            this.items = [];
	            this.saveToLocalStorage();
	        }
    ...
    $scope.order_choose_banking = {
        createDate: new Date(),
        address: "10 Quang Trung",
        ward: "",
        district: "",
        city: "",
        zipcode: "",
        ...
        purchase() {
            ...
            swal({
                ...
                if (purchase) {
                    swal("Vui lòng chọn ngân hàng để thanh toán !", {
                        icon: "success",
                    });
                    $http.post("/rest/orders", order).then(resp => {
                        $scope.cart.clear_in_vnpay();
                        location.href = "/checkout/vnpay/choose_banking";
                    })
                } else {
                    swal("Hủy đơn!");
                }
    ...
    **/
    ...
    $scope.cartFav = {
        ...
        clearFav() {
        	var result = confirm("Bạn có muốn xóa hết phẩm yêu thích này ?");
            if (result) {
	            this.itemsFav = [];
	            this.saveToLocalStorage();
	        }
